...
public class ThreadsController extends Thread {
    ...
    ThreadsController(final Tuple positionDepart) {
    ...
    public void run() {
     ...
     private void spawnFood(Tuple foodPositionIn){
         Window.Grid.get(foodPositionIn.x).get(foodPositionIn.y).lightMeUp(DataOfSquare.GameColor.FOOD);
     }
     ...
     private void moveExterne(){
         for(Tuple t : positions){
             int y = t.getX();
             int x = t.getY();
             Window.Grid.get(x).get(y).lightMeUp(DataOfSquare.GameColor.SNAKE);
     ...
     private void deleteTail(){
         ...
         for(int i = positions.size()-1;i>=0;i--){
             if(cmpt==0){
                 ...
                 Window.Grid.get(t.y).get(t.x).lightMeUp(DataOfSquare.GameColor.BACKGROUND);
             }
             else{
                 cmpt--;
             }
